...
public class VendingMachineServiceImpl implements IVendingMachineService 
{
    ...
    public int insertVendingMachine(VendingMachine vendingMachine)
    {
        ...
        BeanUtil.copyProperties(node,vendingMachine,"id");// 商圈类型 区域id
        ...
        // 保存
        int restult = vendingMachineMapper.insertVendingMachine(vendingMachine);
        // 新增货道
        // 双层for循环
        ...
        for (int i = 1; i <= vmType.getVmRow(); i++){
            for (int j = 1; j <= vmType.getVmCol(); j++){
                ...
                channel.setInnerCode(innerCode+channel.getChannelCode()); // 货道内部编号
    ...
    public int updateVendingMachine(VendingMachine vendingMachine)
    {
        ...
        if(vendingMachine.getNodeId()!=null){
            Node node = nodeService.selectNodeById(vendingMachine.getNodeId());
            BeanUtils.copyProperties(node,vendingMachine,"id"); // 商圈类型 区域id
            vendingMachine.setAddr(node.getAddress()); // 设备地址
        }
