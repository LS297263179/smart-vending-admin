...
//            throw new RuntimeException("员工区域不匹配");
//        }
//        // 将dto转为po并补充属性 保存工单
//        Task task = BeanUtil.copyProperties(taskDto, Task.class);// 拷贝属性
//        task.setTaskStatus(DkdContants.TASK_STATUS_CREATE);// 创建工单
//        task.setUserName(emp.getUserName()); //  设置员工名称
//        task.setRegionId(vm.getRegionId()); // 设置区域id
//        task.setAddr(vm.getAddr()); // 设置售货机地址
...
//                throw new ServiceException("补货商品不能为空");
//            }
//            // 将dto转为po
...
//        // 根据日期生成redis的键
//        String key = "task:code:" + date;
//        // 判断key是否存在
//        if(!redisTemplate.hasKey(key)){
//            // 如果key不存在 设置初始值为1 过期时间为1天
...
//        // 创建task条件对象 并设置编号和工单类型以及工单状态为进行中
//        Task task = new Task();
...
//            throw new RuntimeException("设备有未完成的工单,无法重新创建");
//        }
//    }
...
//    }
    ...
    public int cancelTask(Task task) {
        ...
        task.setUpdateTime(DateUtils.getNowDate()); // 跟心时间

        // 更新工单 这里的task是前端的task参数
        return taskMapper.updateTask(task);
    }
