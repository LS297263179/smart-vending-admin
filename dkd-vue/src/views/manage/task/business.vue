...
const channelDetails = () => {
  proxy.$refs['taskRef'].validateField('innerCode', (error) => {
    console.log('error的值：', error); // 打印错误信息
    if (!error) {
      return;
    }
...
// 补货清单
// const channelDetails = () => {
//   proxy.$refs.taskRef.validateField('innerCode', (err) => {
//     if (err) return;          // 校验失败直接 return
//     channelVisible.value = true;
//     /* 关键点：弹层打开后立刻让子组件重新调接口 */
//     nextTick(() => {
//       replDialog.value?.fetchChannels();   // 调子组件方法重新拉数据
//     });
//   });
// };
